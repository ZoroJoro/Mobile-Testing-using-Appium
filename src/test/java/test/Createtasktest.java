...
public class CreateTaskTest extends BaseTest {
        ...
        private final By TODO_TEXT = AppiumBy.androidUIAutomator("new UiSelector().text(\"Todo\")");

        private final By COMPLETED_FILTER_TEXT = AppiumBy
                        .androidUIAutomator("new UiSelector().text(\"Completed Tasks\")");

        private final By TITLE_INPUT = AppiumBy
                        .androidUIAutomator("new UiSelector().className(\"android.widget.EditText\").instance(0)");

        private final By DESC_INPUT = AppiumBy
                        .androidUIAutomator("new UiSelector().className(\"android.widget.EditText\").instance(1)");

        private final By CONFIRM_BUTTON = AppiumBy
        ...
        public void DeleteTask() {
                ...
                Assert.assertTrue(
                                getWait().until(ExpectedConditions.visibilityOfElementLocated(
                                                AppiumBy.androidUIAutomator(
                                                                "new UiSelector().text(\"Task was deleted\")")))
                                                .isDisplayed(),
        ...
        public void CheckTaskCompleted() {
                ...
                Assert.assertTrue(
                                getWait().until(ExpectedConditions.visibilityOfElementLocated(
                                                AppiumBy.androidUIAutomator(
                                                                "new UiSelector().text(\"Task marked complete\")")))
                                                .isDisplayed(),
